window.onload = function () {
    var estado = window.localStorage.getItem("caja");
    if (estado == "true") {
        document.getElementById("f").style.display = "block";
    }
}

// 判断如果所有单都已经收了钱时，才允许关闭页面
function analizar_cerrar(){

    const alerta = document.getElementById("alerta_cierre");

    for(i=1 ; i<=18 ; i++){

        let obj = "m"+i;
        let analiOBJ = localStorage.getItem(obj);

        if(analiOBJ == "null"){
            continue;
        }else{
            alerta.innerHTML = "antes de cierrar secion , todo los pedido en linea debe estar pagado."
...
function cerrar_sesion() {
    ...
    // 退出时消除当日营业的记录
    let db_lenght = localStorage.getItem("historialDiaNum");
    let obj = indexedDB.open("Bossdatos");
    let db;
    ...
    obj.onsuccess = function (event) {
        ...
        for (i = 1; i <= db_lenght; i++) {
            db.transaction(["Historial_dia"], "readwrite").objectStore("Historial_dia").delete(i);
        }
...
// 两个返回按钮的对象
const boton_regreso1 = document.getElementById("regresar1");
const boton_regreso2 = document.getElementById("regresar2");
...
// 监视点击事件，执行退出支付窗口的函数
boton_regreso1.addEventListener('click', regreso);
boton_regreso2.addEventListener('click', regreso);


/* **************************************  显示当天营业记录的功能  *************************************************************** */

// 该变量接收记录表格中天的对象
const tablaDia = document.getElementById("tabla_dia");

// 该变量接收记录表格中日期的对象
const tablaFecha = document.getElementById("tabla_fecha");

// 该变量接收记录表的对象
const historialTabla = document.getElementById("historial_tabla");
...
function muestraHT() {
    ...
    // 打开对应的数据库
    const historialOBJ = indexedDB.open("Bossdatos");
    let HDdb;

    // 该变量获取当前今日收入记录的长度
    let db_lenght = localStorage.getItem("historialDiaNum");

    // 该变量用来接收当日是星期几字体（lunes ， martes ...）
    let dia;
    ...
    // 该变量用来接收当前是星期几 (0 , 1 , 2, 3 ...)
    let diaNum = TiempoOBJ.getDay();
    ...
    // 获取当天日期
    let fecha = TiempoOBJ.toLocaleDateString();

    // 把日期和星期赋值给表
    tablaDia.innerHTML = dia;
    tablaFecha.innerHTML = fecha;

    // 获取记录内容并展示
    historialOBJ.onsuccess = function (event) {

        HDdb = event.target.result;

        historialTabla.innerHTML = historialTabla.innerHTML + `
        ...
        // 通过循环，把数据库中的每一条数据都展示出来
        for (i = 1; i <= db_lenght; i++) {
            let obtKey = HDdb.transaction(["Historial_dia"], "readwrite").objectStore("Historial_dia").get(i);
            ...
            for (j = 0; j < 100; j++) {
                ...
                cero += "0";
                codigo_constructor = "#" + cero + i;

                if (codigo_constructor.length == 9) {
                    break;
                }
            }

            obtKey.onsuccess = function () {

                historialTabla.innerHTML = historialTabla.innerHTML + `

              
                <tr>
                <td>${obtKey.result.hora}</td>
                <td>${codigo_constructor}</td>
                <td>${"S/. " + obtKey.result.monto}</td>
                <td><button onclick="detallar(${num})">detalles</button></td>
                </tr>
...
// 显示每一条记录的详细信息 num 是 1 , 2 , 3 ....
function detallar(num) {

    console.log(num)

    // 该变量接收提示框的对象
    const cuadro = document.getElementById("cuadro_pago");
    ...
    historialOBJ.onsuccess = function (event) {
        ...
        let obtKey = HDdb.transaction(["Historial_dia"], "readwrite").objectStore("Historial_dia").get(num);

        obtKey.onsuccess = function () {

            let detalles = obtKey.result.detalle;
            // let nuevo_det = detalles.replace(/$/g , " - ");

            const obj = document.getElementById("d"+num);

            obj.innerHTML = detalles ;
            obj.style.color ="red";
        }
